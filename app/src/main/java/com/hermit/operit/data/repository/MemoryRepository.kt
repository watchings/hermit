package com.hermit.data.repository

import android.content.Context
import androidx.compose.ui.graphics.Color
import com.hermit.R
import com.hermit.data.db.ObjectBoxManager
import com.hermit.data.model.Memory
import com.hermit.data.model.MemoryLink
import com.hermit.data.model.MemoryTag
import com.hermit.data.model.MemoryTag_
import com.hermit.data.model.Memory_
import com.hermit.data.model.DocumentChunk
import com.hermit.data.model.Embedding
import com.hermit.data.model.CloudEmbeddingConfig
import com.hermit.data.model.DimensionCount
import com.hermit.data.model.EmbeddingDimensionUsage
import com.hermit.data.model.EmbeddingRebuildProgress
import com.hermit.data.preferences.MemorySearchSettingsPreferences
import com.hermit.services.CloudEmbeddingService
import com.hermit.ui.features.memory.screens.graph.model.Edge
import com.hermit.ui.features.memory.screens.graph.model.Graph
import com.hermit.ui.features.memory.screens.graph.model.Node
...
import com.hermit.data.model.MemoryExportData
import com.hermit.data.model.SerializableMemory
import com.hermit.data.model.SerializableLink
import com.hermit.data.model.ImportStrategy
import com.hermit.data.model.MemoryImportResult
import com.hermit.data.model.MemorySearchDebugCandidate
import com.hermit.data.model.MemorySearchDebugInfo
import com.hermit.data.model.MemoryScoreMode
import com.hermit.util.OperitPaths
import com.hermit.util.TextSegmenter
import com.hermit.util.vector.IndexItem
import com.hermit.util.vector.VectorIndexManager
...
class MemoryRepository(private val context: Context, profileId: String) {
    ...
    suspend fun deleteMemory(memoryId: Long): Boolean = withContext(Dispatchers.IO) {
        ...
        if (memory.isDocumentNode) {
            ...
            if (chunkIds.isNotEmpty()) {
                ...
                com.hermit.util.AppLogger.d("MemoryRepo", "Deleted ${chunkIds.size} associated chunks for document.")
            }
        }
    ...
    suspend fun linkMemories(
        ...
        if (existingLink != null) {
            // 链接已存在，可以选择更新或直接返回
            // 这里我们选择直接返回，不创建重复链接
            com.hermit.util.AppLogger.d("MemoryRepo", "Link already exists from memory ${source.id} to ${target.id} with type $type")
            return@withContext
        }
    ...
    private suspend fun runSearchMemoriesWithDebug(
        ...
        if (memoriesToSearch.isEmpty()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "No memories found in folder '$folderPath' to search.")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = buildDebug(
        ...
        if (reverseContainmentResults.isNotEmpty()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "Reverse containment: ${reverseContainmentResults.size} matches")
        }
        if (effectiveKeywordWeight > 0.0) {
        ...
        if (effectiveSemanticWeight > 0.0f && cloudConfig.isReady()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "--- Starting Semantic Search for ${keywords.size} keywords ---")
            keywords.forEach { keyword ->
                val queryEmbedding = generateEmbedding(keyword, cloudConfig)
                if (queryEmbedding == null) {
                    com.hermit.util.AppLogger.w("MemoryRepo", "Failed to generate embedding for: '$keyword'")
                    return@forEach
                }
            ...
            com.hermit.util.AppLogger.d("MemoryRepo", "--- Semantic Search Completed ---")
        }
        ...
        if (edgesTraversed > 0) {
            com.hermit.util.AppLogger.d("MemoryRepo", "Graph expansion: ${edgesTraversed} edges traversed")
        }
        ...
        com.hermit.util.AppLogger.d("MemoryRepo", "Final results: ${filteredScores.size}/${scores.size} above threshold")
        ...
        sortedScoresForLogging.take(3).forEach { (id, score) ->
            ...
            if (memory != null) {
                com.hermit.util.AppLogger.d("MemoryRepo", "  Top: [${memory.title}] = ${String.format("%.4f", score)}")
            }
        }
        ...
        if (filteredScores.isEmpty()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "No memories above relevance threshold")
            return@withContext SearchComputationResult(
                memories = emptyList(),
                debug = debugInfo
            )
    ...
    suspend fun searchChunksInDocument(memoryId: Long, query: String, limit: Int = 20): List<DocumentChunk> = withContext(Dispatchers.IO) {
        com.hermit.util.AppLogger.d("MemoryRepo", "--- Starting search in document (Memory ID: $memoryId) for query: '$query' ---")
        val memory = findMemoryById(memoryId) ?: return@withContext emptyList<DocumentChunk>().also {
            com.hermit.util.AppLogger.w("MemoryRepo", "Document with ID $memoryId not found.")
        }
        if (!memory.isDocumentNode) {
        ...
        if (query.isBlank()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "Query is blank, returning up to $validLimit chunks sorted by index.")
        ...
        if (effectiveSemanticWeight > 0.0f && cloudConfig.isReady()) {
            keywords.forEach { keyword ->
                val queryEmbedding = generateEmbedding(keyword, cloudConfig)
                if (queryEmbedding == null) {
                    com.hermit.util.AppLogger.w("MemoryRepo", "Failed to generate chunk embedding query for: '$keyword'")
                    return@forEach
                }
        ...
        if (scores.isEmpty()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "No matching chunks found after weighted ranking.")
            return@withContext emptyList()
        }
        ...
        val combinedResults = scores.entries
            ...
            .take(validLimit)
        com.hermit.util.AppLogger.d("MemoryRepo", "Weighted document chunk results count: ${combinedResults.size}. Limit=$validLimit.")
        com.hermit.util.AppLogger.d("MemoryRepo", "--- Search in document finished ---")
    ...
    suspend fun getAllFolderPaths(): List<String> = withContext(Dispatchers.IO) {
        ...
        com.hermit.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Total memories: ${allMemories.size}")
        val folderPaths = allMemories
            .map { normalizeFolderPath(it.folderPath) ?: context.getString(R.string.memory_uncategorized) }
            .distinct()
            .sorted()
        com.hermit.util.AppLogger.d("MemoryRepository", "getAllFolderPaths: Unique folders: $folderPaths")
        folderPaths
    }
    ...
    suspend fun renameFolder(oldPath: String, newPath: String): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            com.hermit.util.AppLogger.e("MemoryRepo", "Failed to rename folder", e)
            false
        }
    }
    ...
    suspend fun moveMemoriesToFolder(memoryIds: List<Long>, targetFolderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.hermit.util.AppLogger.e("MemoryRepo", "Failed to move memories", e)
            false
        }
    }
    ...
    suspend fun createFolder(folderPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            com.hermit.util.AppLogger.e("MemoryRepo", "Failed to create folder", e)
            false
        }
    }
    ...
    suspend fun mergeMemories(
        ...
        if (sourceMemories.size < 2) {
            com.hermit.util.AppLogger.w("MemoryRepo", "Merge requires at least two unique source memories to be found. Found: ${sourceMemories.size} from titles: ${sourceTitles.joinToString()}.")
        ...
        try {
            ...
            com.hermit.util.AppLogger.e("MemoryRepo", "Error during memory merge transaction.", e)
            return@withContext null
        }
    ...
    suspend fun deleteMemoriesByUuids(uuids: Set<String>): Boolean = withContext(Dispatchers.IO) {
        com.hermit.util.AppLogger.d("MemoryRepo", "Attempting to delete memories with UUIDs: $uuids")
        if (uuids.isEmpty()) {
            com.hermit.util.AppLogger.d("MemoryRepo", "UUID set is empty, nothing to delete.")
        ...
        com.hermit.util.AppLogger.d("MemoryRepo", "Found ${memoriesToDelete.size} memories to delete.")
        ...
        try {
            store.runInTx {
                ...
                com.hermit.util.AppLogger.d("MemoryRepo", "Found ${linkIdsToDelete.size} unique links and ${chunkIdsToDelete.size} chunks to delete.")

                // 2. 批量删除链接和区块
                if (linkIdsToDelete.isNotEmpty()) {
                    linkBox.removeByIds(linkIdsToDelete)
                    com.hermit.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${linkIdsToDelete.size} links.")
                }
                if (chunkIdsToDelete.isNotEmpty()) {
                    chunkBox.removeByIds(chunkIdsToDelete)
                    com.hermit.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${chunkIdsToDelete.size} chunks.")
                }

                // 3. 批量删除记忆本身
                memoryBox.removeByIds(memoryIdsToDelete.toList())
                com.hermit.util.AppLogger.d("MemoryRepo", "Bulk-deleted ${memoriesToDelete.size} memories.")
            }
            com.hermit.util.AppLogger.d("MemoryRepo", "Transaction completed successfully.")
        } catch (e: Exception) {
            com.hermit.util.AppLogger.e("MemoryRepo", "Error during bulk delete transaction.", e)
            return@withContext false
        }

        // 4. 在事务外处理向量索引和文件
        memoriesToDelete.forEach(::removeMemoryFromIndexInternal)
        com.hermit.util.AppLogger.d("MemoryRepo", "Removed deleted memories from vector indices and cleaned up chunk index files.")
        ...
        memories.forEach { memory ->
            ...
            memory.links.forEach { link ->
                ...
                if (sourceId != null &&
                    ...
                    // com.hermit.util.AppLogger.d("MemoryRepo", "Discarding edge: $sourceId -> $targetId
    ...
    suspend fun deleteFolder(folderPath: String) {
        withContext(Dispatchers.IO) {
            ...
            com.hermit.util.AppLogger.d("MemoryRepo", "Deleted folder '$folderPath', moved ${memories.size} memories to uncategorized")
        }
    }
    ...
    suspend fun importMemoriesFromJson(
        ...
        try {
            ...
            com.hermit.util.AppLogger.d("MemoryRepo", "Import completed: $newCount new, $updatedCount updated, $skippedCount skipped, $newLinksCount links")
            ...
            com.hermit.util.AppLogger.e("MemoryRepo", "Failed to import memories", e)
            throw e
        }
    }
