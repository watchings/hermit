...
import com.hermit.core.chat.hooks.SummaryHookContext
import com.hermit.core.chat.hooks.SummaryHookRegistry
...
import com.hermit.core.tools.UIPageResultData
import com.hermit.core.tools.SimplifiedUINode
...
import com.hermit.data.preferences.MemorySpaceProfileDocumentRepository
import com.hermit.data.preferences.UserPreferencesManager
import com.hermit.core.avatar.impl.factory.AvatarModelFactoryImpl
import com.hermit.data.repository.AvatarRepository
import com.hermit.util.ChatMarkupRegex
import com.hermit.util.ChatUtils
import com.hermit.core.tools.ToolProgressBus
import com.hermit.util.streamnative.NativeXmlSplitter
...
import com.hermit.core.tools.ComputerDesktopActionResultData
...
import com.hermit.api.chat.llmprovider.MediaLinkBuilder
import com.hermit.data.repository.getCustomMoodDefinitions
import com.hermit.data.repository.getMoodAnimationMapping
    ...
    private suspend fun buildWaifuRulesText(): String {
        ...
        if (waifuEnableEmoticons) {
            ...
            val availableCategories = try {
                ...
                com.hermit.util.AppLogger.e("ConversationService", "获取表情分组失败", e)
                emptyList()
            }
...
${FunctionalPrompts.translationUserPrompt(targetLanguage, text)}
    ...
    suspend fun analyzeImageWithIntent(
        ...
        return try {
            ...
            // 添加图片到池子并获取ID
            val imageId = com.hermit.util.ImagePoolManager.addImage(imagePath)
            if (imageId == "error") {
            ...
            // 清理图片缓存
            com.hermit.util.ImagePoolManager.removeImage(imageId)
    ...
    suspend fun analyzeAudioWithIntent(
        ...
        return try {
            ...
            val mediaId = com.hermit.util.MediaPoolManager.addMedia(audioPath, mimeType)
            if (mediaId == "error") {
            ...
            com.hermit.util.MediaPoolManager.removeMedia(mediaId)
            ChatUtils.removeThinkingContent(result.toString()).trim()
        } catch (e: Exception) {
    ...
    suspend fun analyzeVideoWithIntent(
        ...
        return try {
            ...
            val mediaId = com.hermit.util.MediaPoolManager.addMedia(videoPath, mimeType)
            if (mediaId == "error") {
