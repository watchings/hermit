...
import com.hermit.api.chat.enhance.ConversationMarkupManager
import com.hermit.api.chat.enhance.ConversationRoundManager
import com.hermit.api.chat.enhance.ConversationService
import com.hermit.api.chat.enhance.FileBindingService
import com.hermit.api.chat.enhance.MultiServiceManager
import com.hermit.api.chat.enhance.ToolExecutionManager
import com.hermit.api.chat.llmprovider.AIService
import com.hermit.core.chat.logMessageTiming
import com.hermit.core.chat.messageTimingNow
import com.hermit.core.chat.hooks.PromptHookContext
import com.hermit.core.chat.hooks.PromptHookRegistry
import com.hermit.core.chat.hooks.PromptTurn
import com.hermit.core.chat.hooks.PromptTurnKind
import com.hermit.core.chat.hooks.appendUserTurnIfMissing
import com.hermit.core.chat.hooks.buildActivePromptHookMetadata
import com.hermit.core.chat.hooks.mergeAdjacentTurns
import com.hermit.core.chat.hooks.toPromptTurns
import com.hermit.core.chat.hooks.toRoleContentPairs
import com.hermit.core.application.ActivityLifecycleManager
import com.hermit.core.tools.AIToolHandler
import com.hermit.core.tools.StringResultData
import com.hermit.core.tools.ToolExecutionLimits
import com.hermit.core.tools.climode.CliToolModeSupport
import com.hermit.core.tools.climode.ToolExposureMode
...
import com.hermit.data.model.ToolInvocation
...
import com.hermit.data.preferences.ExternalHttpApiPreferences
import com.hermit.data.preferences.WakeWordPreferences
import com.hermit.util.stream.MutableSharedStream
import com.hermit.util.stream.Stream
import com.hermit.util.stream.StreamCollector
import com.hermit.util.stream.TextStreamEvent
...
import com.hermit.util.stream.withEventChannel
import com.hermit.util.stream.plugins.StreamXmlPlugin
import com.hermit.util.stream.splitBy
import com.hermit.util.stream.stream
import com.hermit.R
...
import com.hermit.data.repository.CustomEmojiRepository
import com.hermit.data.preferences.CharacterCardManager
import com.hermit.data.preferences.CharacterCardToolAccessResolver
import com.hermit.data.preferences.UserPreferencesManager
import com.hermit.data.preferences.preferencesManager
import com.hermit.data.repository.MemoryAutoSaveCandidateRepository
import com.hermit.core.config.SystemToolPrompts
...
class EnhancedAIService private constructor(private val context: Context) {
     ...
     * @param newState 新的状态
     */
    fun setInputProcessingState(newState: com.hermit.data.model.InputProcessingState) {
    ...
    init {
        com.hermit.api.chat.library.MemoryLibrary.initialize(context)
        initScope.launch {
            runCatching {
                ensureInitialized()
            }.onFailure { e ->
    ...
    private suspend fun getModelParametersForFunction(
        ...
        chatModelConfigIdOverride: String? = null,
        chatModelIndexOverride: Int? = null
    ): List<com.hermit.data.model.ModelParameter<*>> {
        ensureInitialized()
        val overrideConfigId = chatModelConfigIdOverride?.takeIf { it.isNotBlank() }
        return if (functionType == FunctionType.CHAT && overrideConfigId != null) {
    ...
    private fun serializePromptHookModelParameters(
        modelParameters: List<com.hermit.data.model.ModelParameter<*>>
    ): List<Map<String, Any?>> {
    ...
    fun saveConversationToMemoryAsync(
        ...
        toolProcessingScope.launch {
            try {
                ...
                com.hermit.api.chat.library.MemoryLibrary.saveMemoryAsync(
                    context = context,
                    toolHandler = toolHandler,
