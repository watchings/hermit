        ...
        return chatHistory.map { turn ->
            ...
            val rawContent =
                if (!preserveThinkInHistory && turn.kind == PromptTurnKind.ASSISTANT) {
                    com.hermit.util.ChatUtils.removeThinkingContent(turn.content)
                } else {
                    turn.content
                }
            val content =
                com.hermit.util.ChatUtils.stripOpenAiResponsesProtocolMarkup(rawContent)
            role to content
        }
    }
    ...
    override suspend fun sendMessage(
        ...
        try {
            ...
            val usageReporter = LocalUsageReporter(
                com.hermit.data.stats.ProviderUsageNormalizer.SOURCE_MNN,
                onUsageReported,
            )
