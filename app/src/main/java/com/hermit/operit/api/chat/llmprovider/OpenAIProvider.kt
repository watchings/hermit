...
import com.hermit.util.HttpLogSanitizer
import com.hermit.util.LocaleUtils
import com.hermit.util.StreamingJsonXmlConverter
import com.hermit.util.TokenCacheManager
import com.hermit.util.exceptions.UserCancellationException
...
open class OpenAIProvider(
    ...
    private suspend fun applyUsageToCounters(
        ...
        onUsageReported: (suspend (com.hermit.data.stats.ProviderUsageSnapshot, attempt: Int) -> Unit)? = null,
        attemptNumber: Int = 1
    ) {
        ...
        onUsageReported?.invoke(
            if (useResponsesApi) {
                com.hermit.data.stats.ProviderUsageNormalizer.openAiResponses(usage)
            } else {
                com.hermit.data.stats.ProviderUsageNormalizer.openAiChatCompletions(usage)
            } ?: return,
            attemptNumber
        )
    }
     ...
     * 从结构化参数构建JSON Schema
     */
    private fun buildSchemaFromStructured(params: List<com.hermit.data.model.ToolParameterSchema>): JSONObject {
    ...
    private inner class StreamEmitter(
        ...
        private val eventChannel: com.hermit.util.stream.MutableSharedStream<TextStreamEvent>,
