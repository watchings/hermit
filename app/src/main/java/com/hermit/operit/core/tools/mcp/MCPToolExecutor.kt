    ...
    fun getOrCreateSession(pluginId: String): McpRuntimeSession? {
        ...
        val session = when (descriptor) {
            is McpRuntimeDescriptor.Local ->
                com.hermit.data.mcp.plugins.BridgeMcpRuntimeSession(context, descriptor.serviceName)
            is McpRuntimeDescriptor.Remote ->
                com.hermit.data.mcp.plugins.RemoteMcpRuntimeSession(pluginId, descriptor)
        }
