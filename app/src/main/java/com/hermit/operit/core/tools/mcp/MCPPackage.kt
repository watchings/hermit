...
import com.hermit.core.tools.LocalizedText
import com.hermit.core.tools.PackageTool
import com.hermit.core.tools.PackageToolParameter
...
data class MCPPackage(
    ...
    companion object {
        ...
        fun loadFromServer(context: Context, serverConfig: MCPServerConfig): LoadResult {
            ...
            com.hermit.util.AppLogger.d(TAG, "正在连接到MCP服务器: ${serverConfig.name}")

            try {
                val connected = runBlocking { session.connect() }
                if (!connected) {
                    com.hermit.util.AppLogger.w(TAG, "无法连接到MCP服务器: ${serverConfig.name}")
                    return LoadResult(
                        mcpPackage = null,
                        errorMessage = mcpManager.getLastConnectionFailureReason(serverConfig.name)
                            ?: "Connection failed"
                ...
                com.hermit.util.AppLogger.d(TAG, "成功连接到MCP服务器: ${serverConfig.name}，开始获取工具列表")

                // 获取工具列表
                val runtimeTools = runBlocking { session.listTools() }
                if (runtimeTools.isEmpty()) {
                    com.hermit.util.AppLogger.w(TAG, "MCP服务器 ${serverConfig.name} 没有提供任何工具")
                    // 不要因为没有工具就返回null
                    // 返回一个包含空工具列表的有效包
                    com.hermit.util.AppLogger.d(TAG, "创建不包含工具的MCP包 - 服务已连接但没有工具")
                    return LoadResult(mcpPackage = MCPPackage(serverConfig, emptyList()))
                }

                com.hermit.util.AppLogger.d(TAG, "成功从MCP服务器获取 ${runtimeTools.size} 个工具")
                ...
                val mcpTools =
                        runtimeTools.mapNotNull { runtimeTool ->
                            try {
                                ...
                                com.hermit.util.AppLogger.e(TAG, "解析MCP工具时出错: ${e.message}")
                                null
                            }
                        }

                com.hermit.util.AppLogger.d(TAG, "成功创建MCP包，包含 ${mcpTools.size} 个工具，保持连接活跃")
                return LoadResult(mcpPackage = MCPPackage(serverConfig, mcpTools))
            } catch (e: Exception) {
                com.hermit.util.AppLogger.e(TAG, "创建MCP包时出错: ${e.message}", e)
                return LoadResult(
                    mcpPackage = null,
